...
import it.antipodi.store.config.KafkaProperties;
...
class AntipodiStoreKafkaResourceIT {
    ...
    void setup() {
        ...
        AntipodiStoreKafkaResource kafkaResource = new AntipodiStoreKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/antipodi-store-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/antipodi-store-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
